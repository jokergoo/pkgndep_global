...
createNetworkFromIgraph(g1)  # this makes figure A

library(png)
p1 = grid.grabExpr({
	grid.text("A) The core dependency graph", x = unit(15, "mm"), y = unit(1, "npc") - unit(2, "mm"), just = c("left", "top"), gp = gpar(fontsize = 13.2))
	grid.raster(readPNG("core-heavy-graph.png"), width = unit(0.85, "snpc"))
})

## distance to leaves
mm = distances(g1, to = V(g1)$name[degree(g1, mode = "out") == 0], mode = "out")
...
createNetworkFromIgraph(gcore)  # figure C
 

library(png)

p4 = grid.grabExpr({
	grid.text("D) Subgraph with edge betweenness >= 20 from the core graph", x = unit(15, "mm"), y = unit(1, "npc") - unit(2, "mm"), just = c("left", "top"), gp = gpar(fontsize = 13.2))
	grid.raster(readPNG("between-core.png"), width = unit(0.85, "snpc"))
})

library(cowplot)
pdf("fig_12.pdf", width = 12, height = 12)
p = plot_grid(p1, p3, p2, p4, nrow = 2)
print(p)
dev.off()
